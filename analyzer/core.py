...
def create_reports(filepath):
    ...
    full_report = {"filename": filename, "mime_type": mime_type, "analysis": {}}
    ...
    try:
        if mime_type and mime_type.startswith("text"):
            ...
            return {
                "summary": f"⚠️ File format '{mime_type}' is not supported.",
                "full": full_report,
            }
        ...
        summary_text += f"🗂 File Operations: {full_report['analysis'].get('file_operations', False)}\n"
        keywords = full_report["analysis"].get("keywords", [])
        summary_text += f"⚠️ Keywords: {', '.join(keywords) if keywords else 'None'}\n"
        heuristics = full_report["analysis"].get("heuristic_flags", [])
        summary_text += (
            f"⚠️ Heuristic Flags: {', '.join(heuristics) if heuristics else 'None'}\n"
        )

        if "pe_info" in full_report["analysis"]:
            pe = full_report["analysis"]["pe_info"]
            if "error" in pe:
                summary_text += f"❌ PE Analysis Error: {pe['error']}\n"
            else:
        ...
        return {"summary": summary_text, "full": full_report}

    except Exception as e:
        return {"summary": f"❌ Error processing file: {str(e)}", "full": full_report}
